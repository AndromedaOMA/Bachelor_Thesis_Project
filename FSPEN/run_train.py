...
class AudioDataset(Dataset):
    def __init__(self, dataset, configs):
        """
        Dataset pentru încărcarea și preprocesarea semnalelor audio.
        :param dataset: Setul de date (ex: 'train' sau 'test').
        :param configs: Configurațiile (inclusiv parametrii pentru preprocesare).
        """
...
def create_dataloader(split, configs, batch_size=32):
    """
    Funcția pentru a crea DataLoader-ul pentru antrenare sau testare.
    :param split: 'train' sau 'test'.
    :param configs: Configurațiile de antrenament.
    :param batch_size: Dimensiunea batch-ului.
    :return: DataLoader
    """
    # Încărcarea setului de date din HuggingFace
    dataset = load_dataset("JacobLinCool/VoiceBank-DEMAND-16k", split=split)

    # Crearea dataset-ului personalizat
    audio_dataset = AudioDataset(dataset, configs)
